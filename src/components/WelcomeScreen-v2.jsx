...
const WelcomeScreen = () => {
  ...
  const { currentDevice, icons } = useGlicoDrive();
  const { Car, Shield, User, ArrowRight, Star } = icons;
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-purple-900 to-blue-700 flex items-center justify-center overflow-hidden rounded-[14px]">
      ...
      >
        {/* Header */}
        <div className="text-center pt-6 sm:pt-8 md:pt-12 pb-3 sm:pb-4 md:pb-6 px-3 sm:px-4 md:px-6 relative flex-shrink-0">
        ...
        <div className="px-3 sm:px-4 md:px-6 pb-4 sm:pb-6 md:pb-8 flex-1 flex flex-col justify-between ">
          <p className="text-gray-600 text-xs sm:text-sm md:text-base lg:text-lg leading-relaxed mb-4 sm:mb-6 px-2 sm:px-4">
            Access comprehensive motor insurance with instant quotes,
            competitive rates, and 24/7 claim support designed for Ghana's
            roads.
          </p>
          {/* Buttons at bottom */}
          ...
          {/* Trust Indicators */}
          <div className="mt-4 sm:mt-6 pt-2 sm:pt-3 border-t border-gray-100 flex-shrink-0">
            <div className="flex items-center justify-center text-xs sm:text-sm text-gray-500 mb-1">
              <Shield className="w-[clamp(16px,2vw,20px)] h-[clamp(16px,2vw,20px)] mr-[2%] text-blue-600" />
